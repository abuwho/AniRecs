...
async def add_preference(
    ...
    try:
        db_user = (
            ...
            .filter(models.User.id == preference.user_id)
            .first()
        )
        db_genre = (
            db.query(models.Genre)
            .filter(models.Genre.id == preference.genre_id)
            .first()
        )
        if not db_user:
            raise HTTPException(
        ...
        if "duplicate key value violates unique constraint" in str(e):
            error_message = (
                "Entry with the same user id and genre id "
                "already exists"
            )
            raise HTTPException(
...
@router.delete(
    "/user/removepreferences/{user_id}/{genre_id}",
...
async def remove_preference(
    ...
    db_preference = (
        ...
        .filter(
            ...
            models.Preference.genre_id == genre_id,
        )
        .first()
    )
