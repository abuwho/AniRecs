...
async def favourite_anime(
    favourite: schemas.FavouriteCreate,
    ...
    try:
        db_user = (
            ...
            .filter(models.User.id == favourite.user_id)
            .first()
        )
        db_anime = (
            db.query(models.Anime)
            .filter(models.Anime.id == favourite.anime_id)
            .first()
        )
        if not db_user:
            raise HTTPException(
        ...
        if "duplicate key value violates unique constraint" in str(e):
            raise HTTPException(
                ...
                detail="User id and anime id already exists",
            )
        else:
            raise
...
@router.delete(
    "/user/removefavourites/{user_id}/{anime_id}",
    status_code=status.HTTP_204_NO_CONTENT,
)
async def unfavourite_anime(
    user_id: int,
    anime_id: int,
    current_user: schemas.UserOut = Depends(current_user),
    db: Session = Depends(database.get_db),
    ...
    db_favourite = (
        ...
        .filter(
            ...
            models.Favourite.anime_id == anime_id,
        )
        .first()
    )
    if not db_favourite:
...
async def get_user_favourited_anime(
    user_id: int,
    current_user: schemas.UserOut = Depends(current_user),
    db: Session = Depends(database.get_db),
):
    ...
    favourites = (
        db.query(models.Anime)
        .join(models.Favourite)
        .filter(models.Favourite.user_id == user_id)
        .all()
    )
    return favourites
